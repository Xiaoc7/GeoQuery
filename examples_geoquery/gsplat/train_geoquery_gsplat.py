...
from datasets.colmap_sparse_depth import Dataset, Parser
...
from examples_geoquery.utils import CameraPoseInterpolator
from src.model import GeoQuery
...
class Config:
    ...
    geoquery_ckpt: Optional[str] = None
    low_res_only: bool = False
    window_size: Optional[int] = None
    
    depth_dir: Optional[str] = None # mvs depth folder path
    ...
    eval_steps: List[int] = field(default_factory=lambda: [1_0000, 2_0000,6_0000])
    # Steps to save the model
    save_steps: List[int] = field(default_factory=lambda: [1_0000, 2_0000, 6_0000])
    # Steps to fix the artifacts
    fix_steps: List[int] = field(default_factory=lambda: [3_000, 6_000, 7000, 8_000, 10_000, 12_000, 14_000, 16_000, 18_000, 20_000, 22_000, 24_000, 26_000, 28_000, 30_000, 32_000, 34_000, 36_000, 38_000, 40_000, 42_000, 44_000, 46_000, 48_000, 50_000, 52_000, 54_000, 56_000, 58_000, 60_000])
    # fix_steps: List[int] = field(default_factory=lambda: [3_000, 6_000, 8_000, 10_000, 12_000, 14_000, 16_000, 18_000, 20_000, 22_000, 24_000, 26_000, 28_000, 30_000, 31_500, 33_000, 34_500, 36_000, 37_500, 39_000, 40_500, 42_000, 43_500, 45_000, 46_500, 48_000, 49_500, 51_000, 52_500, 54_000, 55_500, 57_000, 58_500, 59_500])
...
class Runner:
    ...
    def __init__(
        ...
        self.valset = Dataset(self.parser, split="test")
        ...
        self.geoquery = GeoQuery(
            pretrained_path=cfg.geoquery_ckpt,
            lora_rank_vae=4,
            timestep=199,
            neighborhood_size=cfg.window_size if cfg.window_size is not None else 5,
            low_res_only=cfg.low_res_only,
        )
        self.geoquery.set_eval()
        self.geoquery.to("cuda")
    ...
    def train(self, step=0):
        ...
        if world_rank == 0:
            total_train_time = time.time() - global_tic
            peak_gpu_gb = torch.cuda.max_memory_allocated() / 1024**3
            benchmark_path = f"{cfg.result_dir}/benchmark_results.txt"
            with open(benchmark_path, "w") as f:
                f.write(f"=== Benchmark Results ===\n")
                f.write(f"Total training time: {total_train_time:.2f} s ({total_train_time/3600:.2f} h)\n")
                f.write(f"Peak GPU memory: {peak_gpu_gb:.2f} GB\n")
                f.write(f"Max steps: {max_steps}\n")
                f.write(f"Num GS: {len(self.splats['means'])}\n")
                if len(self.forward_times) > 0:
                    avg_ms = sum(self.forward_times) / len(self.forward_times) * 1000
                    f.write(f"\n=== Diffusion Forward Time ===\n")
                    f.write(f"Total forward calls: {len(self.forward_times)}\n")
                    f.write(f"Avg forward time: {avg_ms:.2f} ms\n")
    ...
    @torch.no_grad()
    def refine_novel_views(self, step: int):
        print("Running GeoQuery refinement...")
        ...
        K = list(self.parser.Ks_dict.values())[0]  # [3, 3]
    
        for i in tqdm.trange(len(novel_poses), desc="Fixing with GeoQuery..."):
            novel_image_path = f"{self.render_dir}/novel/{step}/Pred/{i:04d}.png"
            novel_image = Image.open(novel_image_path).convert("RGB")
        
            ref_train_idx = ref_image_indices[i]
            ref_data = self.trainset[ref_train_idx]
            ref_image_id = ref_data["image_id"]
        
            ref_image_np = ref_data["image"].numpy().astype(np.uint8)
            ref_image = Image.fromarray(ref_image_np)
        
            ref_depth = self.trainset.get_reference_depth(ref_image_id)
        
            if ref_depth is None:
                print(f"Warning: No reference depth for view {ref_image_id}, using original")
                output_image = novel_image
            else:
                ref_pose = ref_data["camtoworld"].numpy()  # [4, 4]
                novel_pose = novel_poses[i]  # [4, 4]
                ref_K = ref_data["K"].numpy()  # [3, 3]
            
                output_image, fwd_time = self.geoquery.sample(
                    target_image=novel_image,
                    reference_image=ref_image,
                    reference_depth=ref_depth,
                    K=ref_K,
                    reference_pose=ref_pose,
                    target_pose=novel_pose,
                )
        ...
        num_novel_views = len(parser.image_paths)
        parser.train_indices = np.arange(num_novel_views, dtype=np.int32)
        parser.test_indices = np.array([], dtype=np.int32)
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        for i, data in enumerate(tqdm.tqdm(valloader)):
            ...
            # print(f"colors: {colors.shape}, alphas: {alphas.shape}")
            torch.cuda.synchronize()
    ...
    def render_traj(self, step: int, camtoworlds_all=None, batch_size=8, tag="novel"):
        ...
        height, width = sample_data["image"].shape[:2]
        ...
        for i in tqdm.trange(0, len(camtoworlds_all), batch_size, desc="Rendering trajectory"):
            ...
            # print(f"renders: {renders.shape}, alphas: {alphas.shape}")
